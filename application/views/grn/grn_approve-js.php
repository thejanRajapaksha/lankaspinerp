...
$(document).ready(function() {
  ...
  manageTable = $('#manageTable').DataTable({
    'ajax': base_url + 'Goodreceive/fetchGoodReceiveDataApprove',
    'order': []
  });
    ...
    $(document).on('click', '#approve_batch', function (e) {
        ...
        if (r == true) {
            ...
            $.ajax({
                url: base_url + 'Goodreceive/approve',
                method: "POST",
                dataType: "json",
                data: {
...
function viewFunc(id) {
    $.ajax({
        url: base_url + 'StyleSchedule/fetchProductOperationsByScheduleId',
        type: 'post',
        dataType: 'json',
        data: {mop_id: id},
        success: function (response) {
            ...
            let res_tr = '<thead><tr><th>Sequence</th><th>Component</th> <th> Operation </th> <th>Machine</th> <th> SMV </th> <th> Target Per Hour </th> <th> Rate </th>  </tr></thead> <tbody>';
            $.each(response.op, function (index, value) {
                let tph = 60 / value.smv
                res_tr += '<tr>' +
