...
class IoModule_model extends CI_Model
{
	...
	public function getDeviceSettings($mac_addr){
		...
		$sql .= "slots.id as slot_id, mlinelist.id as line_id, mbranchlist.profile_id as hrsprofile_id, employee_allocation_current.employee_id ";
	...
	public function getWipLiveData($hrs_profile_id, $inst_id){
		$sql = "select ifnull(drv_wip.id, 0) AS wip_id, ifnull(drv_wip.date, DATE(NOW())) as sch_date, ifnull(drv_hrs.hour_code, drv_wip.hour) as hour, '12345' as sabs_styleid, ifnull(drv_wip.qty, 0) as qty, NOW() as act_time from (select hour_code, hrs_fr, hrs_to from mbranchdayplan where profile_id=? AND (TIME(NOW()) BETWEEN hrs_fr AND hrs_to)) as drv_hrs ";
		...
		$wip_live_query = $this->db->query($sql, array($hrs_profile_id, $inst_id));
